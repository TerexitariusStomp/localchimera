...
const PAYMENT = '200000000000'; // 200 CSPR for contract deploy
const WASM_DIR = '/home/user/CascadeProjects/localchimera/contracts-casper/target/wasm32-unknown-unknown/release';

function loadKey() {
  const pem = readFileSync('/tmp/casper-wallet-new/Account 8_secret_key.pem', 'utf-8');
  ...
  for (let i = 0; i < 120; i++) {
    try {
      ...
      if (info) {
        ...
        if (er && er.cost) {
          console.log(`Deploy ${deployHash} executed successfully (cost: ${er.cost})`);
          return true;
        }
      }
    } catch (e) {
...
async function main() {
  ...
  const crWasm = new Uint8Array(readFileSync(join(WASM_DIR, 'compute_registry_nobulk.wasm')));
  ...
  const crNamedKey = keysAfterCR.find(k => k.name === 'compute_registry_v4_hash');
  if (crNamedKey) {
  ...
  const evWasm = new Uint8Array(readFileSync(join(WASM_DIR, 'escrow_vault_nobulk.wasm')));
  // Build args per-market inside the loop since contract_name differs
  ...
  for (const market of markets) {
    ...
    const marketArgs = sdk.Args.fromMap({
      owner,
      ...
      contract_name: sdk.CLValue.newCLString(market.name),
    });
    const deployHash = await sendDeploy(client, key, evWasm, marketArgs);
    ...
    const effects = deployInfo.executionInfo?.executionResult?.effects || [];
    let contractHash = null;
    for (const e of effects) {
      if (e.kind?.Write && e.key?.startsWith('hash-')) {
        contractHash = e.key.replace('hash-', '');
        break;
      }
    }
    if (contractHash) {
      ...
      const evKey = keysAfter.find(k => k.name === `${market.name}_hash`);
      if (evKey) {
