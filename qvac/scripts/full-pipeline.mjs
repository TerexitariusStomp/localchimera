...
const JOBS_DICT = 'uref-c05482a9ad0d06a70f2f937f2d68d4dbdba49dd7ef6ce5feacdafeaf68a9c2ad-007';
...
async function queryDict(jobId, field) {
  const stateRes = await fetch(RPC_URL, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'chain_get_state_root_hash', params: null })
  }).then(r => r.json());
  const stateRoot = stateRes.result?.state_root_hash;
  const res = await fetch(RPC_URL, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      jsonrpc: '2.0', id: 1, method: 'state_get_dictionary_item',
      params: { state_root_hash: stateRoot, dictionary_identifier: { URef: { seed_uref: JOBS_DICT, dictionary_item_key: `${jobId}:${field}` } } }
    })
  }).then(r => r.json());
...
async function main() {
  ...
  // Step 0: Fund provider account (needs gas for transactions)
  ...
  // Step 1: Consumer deposits to contract purse
  ...
  const jobId = `job:${consumerHash}:0`;
  console.log('Job ID:', jobId);

  // Verify initial job state
  let state = await queryDict(jobId, 'state');
  let jobAmount = await queryDict(jobId, 'amount');
  let jobProvider = await queryDict(jobId, 'provider');
  console.log('Initial state:', state, '| amount:', jobAmount, '| provider:', jobProvider);
  ...
  if (r.status !== 'SUCCESS') return;
  state = await queryDict(jobId, 'state');
  console.log('State after ack:', state);

  // Step 4: Provider completes (the "inference" step)
  ...
  if (r.status !== 'SUCCESS') return;
  state = await queryDict(jobId, 'state');
  let response = await queryDict(jobId, 'response_hash');
  ...
  if (r.status !== 'SUCCESS') return;
  state = await queryDict(jobId, 'state');
  let rating = await queryDict(jobId, 'rating');
  ...
  console.log('Provider received:', Number(providerBalanceAfter) - Number(providerBalanceBefore), 'motes');

  console.log('\n=== FULL PIPELINE COMPLETED SUCCESSFULLY ===');
  console.log('Job lifecycle:');
  console.log('  pending → assigned → provider_done → consumer_confirm → settled');
}
