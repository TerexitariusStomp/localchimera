#!/usr/bin/env python3
"""
QVAC-Chimera v8 — Clean Talking-Head Explainer
Static camera. Same avatar. Screen changes per theme. Hard cuts. No motion.
"""
import subprocess, os
from pathlib import Path
from gtts import gTTS

CLIPS = Path("/home/user/CascadeProjects/qvac-chimera/v8_clips")
CLIPS.mkdir(exist_ok=True)
VIDEO = "/home/user/CascadeProjects/qvac-chimera/qvac_chimera_v8.mp4"
NARRATED = "/home/user/CascadeProjects/qvac-chimera/qvac_chimera_v8_narrated.mp4"
...
concat_txt = CLIPS / "audio_v8_concat.txt"
with open(concat_txt, "w") as f:
    for wav in audio_segments:
        f.write(f"file '{wav}'\n")

full_wav = CLIPS / "full_narration_v8.wav"
run_ffmpeg(["-f", "concat", "-safe", "0", "-i", str(concat_txt), "-c", "copy", str(full_wav)])
print(f"  Full audio: {full_wav.stat().st_size:,}B")

# ── Process clips: static, teal grade, text, hard cuts ───────────────────────
print("\n=== Processing clips (static, no motion) ===")
proc_clips = []
...
for label in labels:
    ...
    # Single pass: teal grade + text. NO zoom, NO fade, NO motion.
    vf = (
...
if not proc_clips:
    print("ERROR: No clips"); exit(1)

# ── Compile with hard cuts (no transitions) ───────────────────────────────────
print(f"\n=== Compiling {len(proc_clips)} clips (hard cuts) ===")
...
padded = CLIPS / "narration_padded_v8.wav"
if pad > 0.5:
...
# ── Viewer ────────────────────────────────────────────────────────────────────
html = Path("/home/user/CascadeProjects/qvac-chimera/view_v8.html")
html.write_text(f"""<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>QVAC-Chimera v8</title>
...
<body>
<h1>QVAC-Chimera — v8 Clean Explainer</h1>
<p>1080×1920 · ~60s · Static camera · Hard cuts · Consistent Avatar · Narrated</p>
<video controls autoplay loop playsinline>
<source src="{Path(NARRATED).name}" type="video/mp4">
</video>
<div class="stats">12 scenes · Same presenter · Screen changes per theme · No motion · No transitions · Comfy Cloud</div>
</body></html>""")
print(f"\n✓ Viewer: {html}")
