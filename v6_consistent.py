#!/usr/bin/env python3
"""
QVAC-Chimera v6 — Consistent Presenter + Changing Screen Content
Uses Kling Image-to-Video with same start_frame for all scenes.
...
CLIPS_DIR = Path("/home/user/CascadeProjects/qvac-chimera/v6_consistent_clips")
CLIPS_DIR.mkdir(exist_ok=True)
FINAL_VIDEO = "/home/user/CascadeProjects/qvac-chimera/qvac_chimera_v6_consistent.mp4"
NARRATED_VIDEO = "/home/user/CascadeProjects/qvac-chimera/qvac_chimera_v6_consistent_narrated.mp4"

# Uploaded base image of presenter + blank screen
BASE_IMAGE = "86b2db9572d403a9ec497de8f41224c84992737fcd9eae05c404ae76ecc213bd.jpg"
...
def make_workflow(screen_content: str) -> dict:
    prompt = f"Screen on monitor now shows {screen_content}. Presenter remains exactly the same. {NEG}"
    return {
        "1": {
            "inputs": {
                "image": BASE_IMAGE,
...
def generate_narration():
    ...
    for label, _ in SCENES:
        wav = CLIPS_DIR / f"{label}_v6c.wav"
    ...
    concat = CLIPS_DIR / "audio_v6c_concat.txt"
    with open(concat, "w") as f:
        for wav in segs:
            f.write(f"file '{wav}'\n")

    full_wav = CLIPS_DIR / "full_narration_v6c.wav"
...
def mix_audio(video: str, audio_wav: Path, out: str):
    ...
    padded = CLIPS_DIR / "narration_padded_v6c.wav"
    if pad > 0.5:
...
# ─── Main ─────────────────────────────────────────────────────────────────────
def main():
    print("=== QVAC-Chimera v6 — Consistent Presenter ===")
    ...
    # Step 6: Viewer
    html = Path("/home/user/CascadeProjects/qvac-chimera/view_v6_consistent.html")
    html.write_text(f"""<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>QVAC-Chimera v6 Consistent</title>
...
<body>
<h1>QVAC-Chimera — v6 Consistent Presenter</h1>
<p>1080×1920 · ~60s · Kling v2.6 Pro · Same Avatar · Changing Screen · Narrated</p>
<video controls autoplay loop playsinline>
<source src="{Path(NARRATED_VIDEO).name}" type="video/mp4">
</video>
<div class="stats">12 scenes · Consistent AI Avatar · Screen content changes · Teal grade · Comfy Cloud</div>
</body></html>""")
    print(f"\n✓ Viewer: {html}")
