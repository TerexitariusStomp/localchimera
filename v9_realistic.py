#!/usr/bin/env python3
"""
QVAC-Chimera v9 — Realistic Presenter, 16:9 Horizontal
Dark studio, warm key light, rim light, shallow depth of field.
Static camera. Hard cuts. Screen changes per theme.
"""
...
CLIPS_DIR = Path("/home/user/CascadeProjects/qvac-chimera/v9_clips")
CLIPS_DIR.mkdir(exist_ok=True)
FINAL_VIDEO = "/home/user/CascadeProjects/qvac-chimera/qvac_chimera_v9.mp4"
NARRATED_VIDEO = "/home/user/CascadeProjects/qvac-chimera/qvac_chimera_v9_narrated.mp4"

BASE_IMAGE = "36629722633731202ea999f23ab762a9f7c54de4e7f531553b339549d59cc112.jpg"

SCENES = [
    ("hook",          "glowing teal sphere pulsing with neon cyan energy in pure black void, cinematic 4K"),
    ("world",         "Earth at night from space with glowing teal neural network lines connecting cities, cinematic"),
    ("problem",       "massive dark data center with cold blue servers, single teal node isolated, cinematic"),
    ("meet_chimera",  "glowing teal network nodes exploding outward from center on black, peer-to-peer mesh topology"),
    ("p2p_mesh",      "hundreds of teal spheres connected by light beams spreading across black space, neural network"),
    ("you_are_node",  "single bright teal glowing sphere with concentric light rings in dark space"),
    ("earning",       "golden coins materializing from teal light streams on black background, cryptocurrency"),
    ("dual_mine",     "split scene: teal AI neural brain and golden blockchain merging with light streams"),
    ("rust_tech",     "teal data packets flowing at light speed through circuit pathways on dark background"),
    ("scale",         "zoom out from single teal node to thousands forming galactic constellation on black"),
    ("future",        "glowing teal humanoid silhouette made of network nodes in darkness, arms outstretched"),
    ("cta",           "pure black screen with single blinking teal terminal cursor, cyan light spreading"),
]

NEG = "watermark, text, logo, low quality, blurry, noise, grain, daylight, bright colors, washed out, nsfw, cartoon, anime"
...
def make_workflow(screen_content: str) -> dict:
    prompt = f"Screen on monitor now shows {screen_content}. Presenter and studio remain exactly the same. {NEG}"
    return {
        ...
        "3": {
            "inputs": {"video": ["2", 0], "filename_prefix": "v9", "format": "mp4", "codec": "h264"},
...
def generate_narration():
    ...
    concat = CLIPS_DIR / "audio_v9_concat.txt"
    with open(concat, "w") as f:
        for wav in segs:
            f.write(f"file '{wav}'\n")

    full_wav = CLIPS_DIR / "full_narration_v9.wav"
...
def mix_audio(video: str, audio_wav: Path, out: str):
    ...
    padded = CLIPS_DIR / "narration_padded_v9.wav"
    if pad > 0.5:
...
def main():
    print("=== QVAC-Chimera v9 — Realistic Presenter 16:9 ===")
    ...
    html = Path("/home/user/CascadeProjects/qvac-chimera/view_v9.html")
    html.write_text(f"""<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>QVAC-Chimera v9</title>
...
<body>
<h1>QVAC-Chimera — v9 Realistic Presenter</h1>
...
<div class="stats">12 scenes · Realistic presenter · Screen changes per theme · Dark studio · Comfy Cloud</div>
</body></html>""")
    print(f"\n✓ Viewer: {html}")
